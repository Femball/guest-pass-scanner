...
BEGIN
  ...
  IF v_kind = 'r' THEN
    ...
    RETURN QUERY
      SELECT r.qr_code, r.client_name, r.event_date, NULL::text,
             (r.number_of_persons::text || ' personne' || CASE WHEN r.number_of_persons > 1 THEN 's' ELSE '' END)
  ...
  ELSIF v_kind = 's' THEN
    ...
    RETURN QUERY
      SELECT b.qr_code,
             ...
             e.event_date,
             e.event_time,
             (b.number_of_persons::text || ' personne' || CASE WHEN b.number_of_persons > 1 THEN 's' ELSE '' END)
